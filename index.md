...
Our best performing model achieved 55% accuracy at 16 genre classification, which was significantly better than our baseline model. We also note that we performed better than the 36% obtained by [this project from CMU] (
http://dawenl.github.io/files/FINAL.pdf) who worked on the same dataset with a similar problem statment but with a different subset. We were also able to generate song embeddings, which were both useful and learned interesting features -- e.g. our model discovered that songs by latin singers are close to each other even though they may belong to different genres. We also discoved that our model often cross-classies rock, punk and metal, which is to be expected since songs from these genres may be very similar and a human may have troubles discerning them.

We note that genre classification given audio data [1] or lyric data [8] and genre embeddings from genre classification are well studied research problem due to their importance in recommendation systems. However, to the best of our knoweldge, all three have not been attempted on the Million Song Dataset (MSD). Furthermore, the team which attempted genre classification on the MSD performed worse. 
